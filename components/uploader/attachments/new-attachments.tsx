import { ListAllAttachmentsInBucket, PushAttachments } from '@/lib/storage-operations'
...
const UploadNewAttachments: FC<{
  ...
  const [uploadedAttachments, setUploadedAttachments] = useState<ExternalFile[]>([])
  ...
  return (
    <div className=" w-full">
      <ReactImageUploading multiple value={attachments} onChange={onChange} acceptType={['docx', 'pdf']} allowNonImageType={true}>
          ...
          <div className="upload__image-wrapper">
            ...
            >
              <label htmlFor="file-upload" className="z-20 flex flex-col-reverse items-center justify-center w-full h-full cursor-pointer">
                <p className="text-xs  text-gray-500">File should be of format .pdf or .doc</p>
                <p className="text-xs  text-gray-500 mt-2">File maximum size is 300Kb</p>
                ...
                {data?.map((file: any, index: any) => (
                  <Card key={index}>
                    <CardBody>
                      ...
                      <span className="text-xs">{formatBytes(file?.metadata?.size ?? 0, 2)}</span>
                      {selectedAttachments?.find((x) => x.name == file.name) ? (
                        <Button
                          ...
                          color="danger"
                          onClick={() => {
                            setselectedAttachments([...selectedAttachments.filter((x) => x.name !== file.name)])
                          }}
                        >
                          Unselect File
                        </Button>
