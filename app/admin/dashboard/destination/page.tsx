      ...
      <div className=" h-full flex-1 flex-col space-y-8 p-8 flex">
        <div className="flex items-center justify-between space-y-2">
          <div>
            ...
            <p className="text-muted-foreground text-sm">Here&apos;s a list of your customer destinations!</p>
          </div>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3  xl:grid-cols-4 gap-x-4 gap-y-8">
          <CardAdd trigger="onOpenDestination" title="Click to create new customer destination" />
          {data?.results
            ?.filter((x) => !x.is_office)
            .map((location) => (
